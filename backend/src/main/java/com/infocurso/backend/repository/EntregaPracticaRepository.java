...
""")
    ...
    @Query("SELECT COUNT(e) FROM EntregaPractica e WHERE e.alumno.id = :alumnoId AND e.practica.curso.id = :cursoId")
    long contarEntregasAlumno(UUID alumnoId, UUID cursoId);
}
